...
const SidebarRestaurantMobile = () => {
    const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
    const navigate = useNavigate();
    const location = useLocation();

    const menusHaut = [
        { label: 'Accueil', shortLabel: 'Home', icon: LayoutDashboard, path: '/dashboard/restaurant', color: 'from-blue-500 to-blue-600' },
        { label: 'Menus', shortLabel: 'Menu', icon: BookOpen, path: '/dashboard/restaurant/menus', color: 'from-emerald-500 to-emerald-600' },
        { label: 'Commandes', shortLabel: 'Orders', icon: ShoppingCart, path: '/dashboard/restaurant/commandes', color: 'from-orange-500 to-orange-600' },
    ];

    const menusBas = [
        { label: 'Paramètres', shortLabel: 'Config', icon: Settings, path: '/dashboard/restaurant/parametres', color: 'from-gray-500 to-gray-600' },
        { label: 'Notifications', shortLabel: 'Notifs', icon: Bell, path: '/dashboard/restaurant/notifications', color: 'from-red-500 to-red-600', badge: 3 },
    ];
        ...
        <div className="relative">
                ...
                <div className="px-5 pb-8 max-h-[85vh] overflow-y-auto">
                    ...
                    <div className="flex items-center justify-between py-5 border-b border-gray-100">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="font-bold text-xl text-gray-900">Mon Restaurant</h3>
                                <p className="text-sm text-gray-500 mt-0.5">Gestion culinaire</p>
                            </div>
                        </div>
                        <button
                            onClick={() => setMobileMenuOpen(false)}
                    ...
                    <div className="border-t border-gray-100 pt-6 mt-6">
                        <div className="flex items-center gap-4 p-4 rounded-2xl bg-gradient-to-r from-gray-50 to-gray-100">
                            <div className="bg-gradient-to-r from-purple-500 to-pink-500 w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-lg shadow-sm">
                                MR
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="font-bold text-gray-900 text-lg">Mon Restaurant</p>
                                <p className="text-sm text-gray-500 truncate mt-0.5">restaurant@example.com</p>
                            </div>
            ...
            <div className="fixed bottom-0 inset-x-0 h-20 bg-white border-t border-gray-100 z-40 safe-area-pb">
                <div className="flex items-center justify-between h-full px-3">
                    ...
                    <button
                        onClick={() => handleRouteClick('/dashboard/restaurant')}
                    ...
                    >
                        <LayoutDashboard className={`w-6 h-6 mb-1 flex-shrink-0 ${isActive('/dashboard/restaurant') ? 'text-orange-500' : ''}`} />
                        <span className={`text-xs font-medium truncate w-full text-center leading-tight ${isActive('/dashboard/restaurant') ? 'text-orange-500' : ''}`}>Accueil</span>
                    </button>

                    <button
                        onClick={() => handleRouteClick('/dashboard/restaurant/menus')}
                        className="flex flex-col items-center justify-center py-2 px-2 rounded-xl transition-all duration-200 min-w-0 flex-1 max-w-[70px] text-gray-400 hover:text-gray-600"
                    >
                        <BookOpen className={`w-6 h-6 mb-1 flex-shrink-0 ${isActive('/dashboard/restaurant/menus') ? 'text-orange-500' : ''}`} />
                        <span className={`text-xs font-medium truncate w-full text-center leading-tight ${isActive('/dashboard/restaurant/menus') ? 'text-orange-500' : ''}`}>Menus</span>
                    </button>
                    ...
                    <button
                        onClick={() => handleRouteClick('/dashboard/restaurant/commandes')}
                    ...
                    >
                        <ShoppingCart className={`w-6 h-6 mb-1 flex-shrink-0 ${isActive('/dashboard/restaurant/commandes') ? 'text-orange-500' : ''}`} />
                        <span className={`text-xs font-medium truncate w-full text-center leading-tight ${isActive('/dashboard/restaurant/commandes') ? 'text-orange-500' : ''}`}>Orders</span>
                        <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full shadow-sm"></div>
                    </button>

                    <button
                        onClick={() => handleRouteClick('/dashboard/restaurant/parametres')}
                        className="flex flex-col items-center justify-center py-2 px-2 rounded-xl transition-all duration-200 min-w-0 flex-1 max-w-[70px] text-gray-400 hover:text-gray-600"
                    >
                        <Settings className={`w-6 h-6 mb-1 flex-shrink-0 ${isActive('/dashboard/restaurant/parametres') ? 'text-orange-500' : ''}`} />
                        <span className={`text-xs font-medium truncate w-full text-center leading-tight ${isActive('/dashboard/restaurant/parametres') ? 'text-orange-500' : ''}`}>Config</span>
                    </button>
                </div>
            </div>
...
export default SidebarRestaurantMobile;